...
Deno.test('pricing quote basic', async () => {
  ...
  const r = await fetch(`${BASE}/pricing`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(body)});
...
Deno.test('kyc_webhook unauthorized without secret', async () => {
  const r = await fetch(`${BASE}/kyc_webhook`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ profile_id:'x', status:'pending' }) });
  if(r.status !== 401 && r.status !== 500){ // allow 500 if secret missing in deployed env
