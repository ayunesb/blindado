...
serve(async (req) => {
  ...
  if (!booking_id || typeof booking_id !== "string") return j({ error: "booking_id required" }, 400);

  const { error } = await admin
    .from("quotes")
    .upsert({ booking_id, payload: quote, created_at: new Date().toISOString() }, { onConflict: "booking_id" });
