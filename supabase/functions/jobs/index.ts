  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (bookingId) {
        ...
        return new Response(
          JSON.stringify({ status: asg.status, accepted, assignment_id: asg.id, last_heartbeat }),
          { headers: cors },
        );
      }
    }
    ...
    if (req.method === 'GET' && url.pathname.endsWith('/list')) {
      ...
      if (!guard_id)
        return new Response(JSON.stringify({ error: 'guard_id required' }), {
          status: 400,
          headers: cors,
        });
      ...
      const cIds = Array.from(
        new Set(bookings.filter((b) => bIds.includes(b.id)).map((b) => b.client_id)),
      );
      ...
      const guardIds = Array.from(new Set(assigns.map((a) => a.guard_id))).filter(
        Boolean,
      ) as string[];
      let licByGuard: Record<
        string,
        Array<{ type: string; status: string; valid_to: string | null }>
      > = {};
      if (guardIds.length) {
      ...
      const items = assigns.map((a) => {
        ...
        const badges = (licByGuard[a.guard_id] || []).map((b) => ({
          label: b.type,
          status: b.status,
          valid_to: b.valid_to,
        }));
    ...
    if (req.method === 'POST' && url.pathname.endsWith('/accept')) {
      ...
      // first-accept wins
      const { data: a, error: e0 } = await supa
        .from('assignments')
      ...
      if (e0) throw e0;
      if (a.status !== 'offered')
        return new Response(JSON.stringify({ ok: true, info: 'already accepted' }), {
          headers: cors,
        });

      const { error: e1u } = await supa
        .from('assignments')
        .update({ status: 'accepted' })
        .eq('id', assignment_id);
      if (e1u) throw e1u;
      const { error: e2u } = await supa
        .from('bookings')
