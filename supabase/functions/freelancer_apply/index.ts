...
serve(async (req) => {
  ...
  await supa
    .from('profiles')
    .upsert({ user_id: user.id, role: 'freelancer' }, { onConflict: 'user_id' });
  const g = await supa
    .from('guards')
    .upsert(
      {
    ...
    .single();
  if ((g as any).error)
    return new Response(JSON.stringify({ error: (g as any).error.message }), {
      status: 400,
      headers: cors,
    });
  for (const d of documents || []) {
    await supa
      ...
      .insert({ guard_id: (g as any).data.id, doc_type: d.type, url: d.url });
  }
