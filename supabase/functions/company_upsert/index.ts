...
serve(async (req) => {
  ...
  const up = await supa.from("companies").upsert({ id, name, tax_id, contact_name, contact_email, contact_phone, cities, status, created_by: user.id }).select().single();
  if ((up as any).error) return new Response(JSON.stringify({ error: (up as any).error.message }), { status: 400, headers: cors });
  await supa.from("profiles").upsert({ user_id: user.id, role: "company_admin", company_id: (up as any).data.id }, { onConflict: "user_id" });
