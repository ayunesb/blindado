  ...
  try {
    if (req.method === 'POST') {
      ...
      if (body?.op === 'consume') {
        ...
        if (inv.used)
          return new Response(JSON.stringify({ error: 'token used' }), {
            status: 400,
            headers: CORS,
          });
        ...
        const { error: e2 } = await supa
          .from('org_members')
          .insert({ org_id: inv.org_id, profile_id: user_id, role: inv.role });
        if (e2)
        ...
        return new Response(JSON.stringify({ ok: true, org_id: inv.org_id, role: inv.role }), {
          headers: CORS,
        });
      }
      ...
      if (!org_id || !email)
        return new Response(JSON.stringify({ error: 'org_id and email required' }), {
          status: 400,
          headers: CORS,
        });
      ...
      const { error } = await supa
        .from('org_invites')
        .insert({ token, org_id, email, role, expires_at });
      if (error) throw error;

      const link = `${INVITE_BASE_URL}?token=${encodeURIComponent(token)}`;
      await sendEmailResend(
        email,
        'Invitación a Blindado',
        `<p>Has sido invitado a Blindado. Únete:</p><p><a href="${link}">${link}</a></p><p>Expira: ${new Date(expires_at).toLocaleString()}</p>`,
      );
