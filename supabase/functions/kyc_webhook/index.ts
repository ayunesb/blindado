...
serve(async (req) => {
  ...
  if (req.headers.get('x-kyc-secret') !== secret)
    return new Response(JSON.stringify({ error: 'unauthorized' }), { status: 401, headers: cors });

  try {
    const { profile_id, status } = await req.json();
    if (!profile_id || !['pending', 'verified', 'rejected'].includes(status))
      return new Response(JSON.stringify({ error: 'profile_id + status required' }), {
        status: 400,
        headers: cors,
      });
