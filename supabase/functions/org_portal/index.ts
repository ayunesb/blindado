...
serve(async (req) => {
  try {
    ...
    if (req.method === 'POST' && path === 'vehicles_upsert') {
      ...
      const { data, error } = await supa.from('vehicles').insert(row).select('id').single();
    ...
    if (req.method === 'POST' && path === 'licenses_upload_base64') {
      ...
      if (!license_id || !guard_id || !b64)
        return json({ error: 'license_id, guard_id, base64 required' }, 400);
      ...
      const { error: updErr } = await supa.from('licenses').update({ files }).eq('id', license_id);
    ...
    if (req.method === 'POST' && path === 'licenses_upload_signed_url') {
      ...
      return json({
        ok: true,
        path: objectPath,
        note: 'Use backend to perform actual upload then append to license.files',
      });
    }
