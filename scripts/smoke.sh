...
echo "→ pricing"
PRICING=$(curl -fsS -X POST "$FN/pricing" -H 'content-type: application/json' -d "$QUOTE_JSON")
echo "$PRICING"
...
echo
echo "→ bookings"
BOOKING_RES=$(curl -fsS -X POST "$FN/bookings" -H 'content-type: application/json' -d "$CREATE_JSON")
echo "$BOOKING_RES"
...
curl -fsS -X POST "$FN/payments_preauth" -H 'content-type: application/json' \
  -d "{\"booking_id\":\"$BOOKING_ID\",\"amount\":$AMOUNT}" | jq .

# AT-4 Confirm & Match
echo
echo "→ bookings_confirm"
curl -fsS -X POST "$FN/bookings_confirm" -H 'content-type: application/json' \
  -d "{\"booking_id\":\"$BOOKING_ID\"}" | jq .

echo
echo "→ matching"
curl -fsS -X POST "$FN/matching" -H 'content-type: application/json' \
  -d "{\"booking_id\":\"$BOOKING_ID\"}" | jq .

# AT-5 Jobs list & accept
echo
echo "→ jobs/list"
LIST=$(curl -fsS "$FN/jobs/list?guard_id=$GUARD_ID")
echo "$LIST" | jq .
...
echo
echo "→ jobs/accept"
curl -fsS -X POST "$FN/jobs/accept" -H 'content-type: application/json' \
...
for s in check_in on_site in_progress check_out completed; do
  echo
  echo "→ jobs/status $s"
  curl -fsS -X POST "$FN/jobs/status" -H 'content-type: application/json' \
